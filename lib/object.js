...
function createObject(obj, opts) {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        // use Maps() here?
        var keyString = new types.StringLiteral([new Token('String', key)]);
...
function getKeysFromObject(obj) {
    return obj.properties.map(function(prop) { return prop.key; });
}

function getValuesFromObject(obj) {
    return obj.properties.map(function(prop) { return prop.value; });
}

function getPropFromObjectByKeyName(obj, key) {
    var props = obj.properties;
    for (var i = 0; i < props.length; i++) {
        var prop = props[i];
        if (prop.key.value === key.toString()) {
            return prop;
        }
    }
    return null;
